ActiveAdmin.register Ingest::Server do
  ...
  index do
    ...
    column :actions do |resource|
      ...
      (resource.aasm(:default).events(permitted: true).map(&:name) - []).each do |event|
        links += link_to event.to_s.humanize, switch_admin_ingest_server_path(resource, switch_params.merge(:event => event)),
  ...
  member_action :switch do
    ...
    redirect_to admin_ingest_servers_path(switch_params)
  end
end
