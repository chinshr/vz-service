...
WaveSurfer.util.extend(WaveSurfer.Drawer.MultiCanvas, {
    ...
    addCanvas: function () {
        ...
        entry.index = leftOffset / this.maxCanvasElementWidth;
    ...
    drawLineToContext: function (ctx, index, peaks, absmax, halfH, scale, offsetY) {
        ...
        var length = ~~(peaks.length * (this.maxCanvasWidth / this.width));

        ctx.beginPath();
        ctx.moveTo(this.halfPixel, halfH + offsetY);

        for (var i = 0; i < length; i++) {
            var h = Math.round(peaks[(length * index) + 2 * i] / absmax * halfH);
            ctx.lineTo(i * scale + this.halfPixel, halfH - h + offsetY);
        }

        // Draw the bottom edge going backwards, to make a single
        // closed hull to fill.
        for (var i = length - 1; i >= 0; i--) {
            var h = Math.round(peaks[(length * index) + 2 * i + 1] / absmax * halfH);
            ctx.lineTo(i * scale + this.halfPixel, halfH - h + offsetY);
        }
