...
class Api::Account::UploadsControllerTest < ActionController::TestCase
  ...
  context "POST /api/account/uploads" do
    ...
    should "NOT create media upload when signed out" do
      sign_out :user
      post :create, params: {:upload => {:file_name => "i-like-pickles.wav", :source_url => "http://s3.amazonaws.com/vz-test-dropbox/8enYwMjB0B",
    ...
    should "NOT create media upload without file_type audio" do
      post :create, params: {:upload => {file_type: "XXX", file_name: "xxx.xxx", file_size: 1,
  ...
  context "GET /api/account/uploads" do
    ...
    context "filters" do
      should "#any_of_status" do
        ...
        get :index, params: {:any_of_status => [Ingest::STATES[:stopped]], format: :json}
        assert_response :success
        assert response_body.has_key?("uploads"), "should have root"
      ...
      should "#none_of_status" do
        @upload2.ingest.update_attribute(:aasm_state, "stopped")
        get :index, params: {:none_of_status => [Ingest::STATES[:stopped]], format: :json}
        assert_response :success
        assert response_body.has_key?("uploads"), "should have root"
        assert_equal 1, response_body["uploads"].size
        assert_attributes response_body["uploads"].first
      ...
      context "sort_order" do
        should "sort all ASC by values #id, #created_at" do
          get :index, params: {:sort_order => ["id", "created_at"], format: :json}
          assert_response :success
          assert response_body.has_key?("uploads"), "should have root"
        ...
        should "sort values and order #id ASC, #created_at DESC" do
          get :index, params: {:sort_order => [{"id" => "asc", "created_at" => "desc"}], format: :json}
          assert_response :success
          assert response_body.has_key?("uploads"), "should have root"
          assert_equal 2, response_body["uploads"].size
        end
  ...
  context "GET /api/account/uploads/:id" do
    should "get upload with :id" do
      ...
      get :show, params: {:id => upload.id, format: :json}
      assert_response :success
      assert_response_body_attributes_with "upload"
    end
    ...
    should "get 404 not found error with invalid :id" do
      ...
      get :show, params: {:id => other_upload.id, format: :json}
  ...
  context "PUT /api/account/uploads/:id" do
    ...
    should "NOT update upload when signed out" do
      sign_out :user
      put :update, params: {:id => 1, :upload => {:title => "La fiesta!", :description => "Entrevista en la fiesta.", locale: "es-AR"}, format: :json}
      assert_response :unauthorized
    end
    ...
    should "invoke remove event" do
      upload = FactoryGirl.create(:media_upload_as_audio)
      upload.user = @user and upload.save
      put :update, params: {:id => upload.id, :upload => {:event => "remove"}, format: :json}
      assert_response :success
      assert_response_body_attributes_with "upload"
  ...
  context "GET /api/account/upload/signed_s3_put" do
    should "get signed_s3_put" do
      get :signed_s3_put, params: {:format => :json, :object_key => "abcd1234", :object_content_type => "image/jpeg"}
      assert_response :success
    ...
    should "NOT get signed_s3_put without object_key parameter" do
      get :signed_s3_put, params: {:format => :json}
      assert_response :unprocessable_entity
    ...
    should "NOT get signed_s3_put without object_content_type parameter" do
      get :signed_s3_put, params: {:format => :json, :object_key => "abcd1234"}
      assert_response :unprocessable_entity
