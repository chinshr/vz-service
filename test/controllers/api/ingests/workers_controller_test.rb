...
class Api::Ingests::WorkersControllerTest < ActionController::TestCase
  ...
  context "POST /api/ingests/:ingest_id/workers.json" do
    context "as backend user" do
      ...
      should "#create" do
        messages = {"error"=>"RuntimeError", "message"=>"YoutubeDL did not like this source 'https://soundcloud.com/evan-desimone-434851586/viacom-vantage-podcast-1'"}
        assert_difference "Ingest::Worker.count", 1 do
          post :create, params: {ingest_id: @ingest.id, worker: {
            ingest_iteration: 1,
  ...
  context "GET /api/ingests/:ingest_id/workers/count(.:format)" do
    should "be unauthorized whithout any user" do
      get :count, params: {ingest_id: @ingest.id, format: :json}
      assert_response :unauthorized
    end
  ...
  context "GET /api/ingests/:ingest_id/workers/:id" do
    ...
    should "be unauthorized whithout any user" do
      get :show, params: {:ingest_id => @ingest.id, :id => @worker1.id, format: :json}
      assert_response :unauthorized
    end
  ...
  context "PUT /api/ingests/:ingest_id/workers/:id(.:format)" do
    ...
    should "be unauthorized whithout any user" do
      put :update, params: {:ingest_id => @ingest.id, :id => @worker1.id, format: :json}
      assert_response :unauthorized
    end
    ...
    context "as backend user" do
      ...
      should "update worker to start with #event=" do
        ...
        put :update, params: {:ingest_id => @ingest.id, :id => @worker1.id, :worker => {
          event: "start",
    ...
    should "not update unknown worker" do
      sign_in @user2, scope: :user
      put :update, params: {:ingest_id => -1, :id => -1, format: :json}
      assert_response :not_found
    end

    should "not update worker of different ingest" do
      sign_in @user2, scope: :user
      put :update, params: {:ingest_id => -1, :id => @worker1.id, format: :json}
      assert_response :not_found
    end
  end
  ...
  context "DELETE /api/ingests/:ingest_id/workers/:id(.:format)" do
    ...
    should "be unauthorized without user" do
      delete :destroy, params: {ingest_id: @ingest, id: @worker1.id, format: :json}
      assert_response :unauthorized
    end
    ...
    should "destroy with backend user" do
      sign_in @user2, scope: :user
      assert_difference "Ingest::Worker.count", -1 do
        delete :destroy, params: {ingest_id: @ingest.id, id: @worker1.id, format: :json}
        assert_response :success
        assert_attributes response_body["worker"]
      end
    end
